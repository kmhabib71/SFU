...
async function handlePeerConnection(socketId, offer) {
  ...
  peerConnection.ontrack = (event) => {
    ...
    console.log("Stream tracks before forwarding:");
    stream
      .getTracks()
      .forEach((t) =>
        console.log(
          `Track ID: ${t.id}, Kind: ${t.kind}, ReadyState: ${t.readyState}`
        )
      );
  ...
  deferredTracks.forEach(({ track, stream }) => {
    ...
    console.log(`Forwarding track ${track.id} with kind ${track.kind}`);
    console.log(`Stream tracks forwarded:`);
    stream
      .getTracks()
      .forEach((t) =>
        console.log(
  ...
  Object.keys(clients).forEach((socketId) => {
    if (socketId !== sourceSocketId && clients[socketId].peerConnection) {
      ...
      try {
        ...
        const existingReceivers = receiverConnection.getReceivers();
        const trackAlreadyAdded = existingReceivers.some(
          (receiver) => receiver.track?.id === track.id
        );
        ...
        if (!trackAlreadyAdded) {
          ...
          console.warn(
            `Track ${track.id} is already added to receiver ${socketId}`
          );
        }
      } catch (error) {
        console.error(
